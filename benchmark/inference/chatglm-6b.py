...
# input_ids = tensor([[ 28815, 130001, 130004]], device='cuda:0')
# position_ids = tensor([[[0, 1, 1],
#          [0, 0, 1]]], device='cuda:0')
# attention_mask = tensor([[[[False, False,  True],
#           [False, False,  True],
#           [False, False, False]]]], device='cuda:0')
    ...
    if mode == "prefill":
        ...
        position_ids = torch.tensor([[list(range(args.prefill_len - 1)) + [args.prefill_len - 2], [0] * (args.prefill_len - 1) + [1]]], 
                                    dtype=torch.int32).cuda()
        attention_mask = torch.tensor([[[([False] * (args.prefill_len - 1) + [True])] * (args.prefill_len - 1) + [[False] * args.prefill_len]]]).cuda()
        past_key_values = None
    else:   # mode == "decode"
        input_ids = inputs.input_ids[:, :1].cuda()
        position_ids = torch.tensor([[[1], [2]]]).cuda()
        attention_mask = None
